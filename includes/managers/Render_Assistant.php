...
class Render_Assistant {
	...
	protected function init_process() {
		add_filter( 'render_block_data', [ $this, 'inject_render_data' ], 10, 1 );
	}
	...
	public function inject_render_data( $block ) {
		...
		if ( $is_plugin_block ) {
			...
			if ( isset( $defaultValues[ $block_name ]['attributes'] ) ) {
				...
				$parsed_default_attrs = array_reduce( array_keys( $block_default_attrs ),
				...
				$block['attrs'] = array_merge( $block['attrs'], $parsed_default_attrs );
			}
		}
