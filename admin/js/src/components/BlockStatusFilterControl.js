...
import ActiveFilterIndicator, {
	generateIndicatorData,
} from '$Components/ActiveFilterIndicator';
...
function BlockStatusFilterControl( { filterVal, onFilterChanged = () => {} } ) {
	...
	const [ indicatorPosData, setIndicatorPosData ] = useState(
		generateIndicatorData( 0, 50 )
	);
	...
	useEffect( () => {
		if ( activeItemRef ) {
			...
			setIndicatorPosData(
				generateIndicatorData( x - containerX, width )
			);
		}
	}, [ activeItemRef ] );
