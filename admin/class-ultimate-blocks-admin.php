...
class Ultimate_Blocks_Admin {
	...
	public function on_admin_init() {
		if ( ! in_array( 'gutenberg/gutenberg.php', get_option( 'active_plugins' ), true ) ) {
			...
			wp_die( 'You must have <strong>Gutenberg Plugin</strong> installed to activate/use <strong>Ultimate Blocks</strong> plugin. If you have Gutenberg already installed please check if it\'s  activated. We are redirecting in <span id="seconds">5</span> seconds... <script> console.log(location.pathname); setTimeout(function() { window.location.href = location.pathname.replace("admin.php", "plugins.php"); document.getElementById("seconds").text = "a"}, 5000);</script>' );
