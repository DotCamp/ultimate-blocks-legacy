...
function ub_register_content_toggle_panel_block() {
	if ( function_exists( 'register_block_type' ) ) {
		...
		register_block_type( dirname(dirname(dirname(__DIR__))) . '/dist/blocks/content-toggle/components/panel/block.json', array(
...
function ub_register_content_toggle_block() {
	if ( function_exists( 'register_block_type' ) ) {
		require dirname( dirname( __DIR__ ) ) . '/defaults.php';
		register_block_type( dirname(dirname(dirname(__DIR__))) . '/dist/blocks/content-toggle/block.json',
			array(
	...
	if ( isset( $block['attrs']['hasFAQSchema'] ) ) {
		...
		foreach ( $block['innerBlocks'] as $key => $togglePanel ) {
			if ( isset( $panel[ $key ] ) ) {
				...
				while ( preg_match_all( '/<([a-z1-6]+)[^>]*?><\/(\1)>/i',
						$answer ) > 0 ) { //remove empty tags and tags that only contain empty tags
				...
				$answer = preg_replace_callback( '/<[a-z1-6]+( (?:(?:aria|data)-[^\t\n\f \/>"\'=]+|[a-z]+)=[\'"][\s\S]+?[\'"])>/i',
					function ( $matches ) {
						$attributeList = preg_replace_callback( '/ ([\S]+)=([\'"])([\s\S]*?)(\2)/',
							function ( $matches ) {
				...
				if ( $answer !== "" && array_key_exists( 'panelTitle',
						$togglePanel['attrs'] ) && $togglePanel['attrs']['panelTitle'] !== '' ) { //blank answers and questions are invalid
 ...
 */
function content_toggle_panel_context( $current_context, $current_block, $parent_block ) {
	if ( $current_block['blockName'] === 'ub/content-toggle-panel-block' ) {
		if ( ! isset( $current_context["parentID"] ) ) {
