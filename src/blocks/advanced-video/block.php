...
function ub_render_advanced_video_block($attributes){
    ...
    ('<div class="ub-advanced-video-thumbnail" style="height:' . esc_html($height) .'px; width:' . esc_attr($width) . '%;">' .
        '<img class="ub-advanced-video-thumbnail-image" height="100%" width="100%" src="' . esc_url($thumbnail) . '">' .
        '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 60 60" xml:space="preserve" width="' . esc_attr($width / 10) . '%">' .
    ...
    . esc_attr($videoEmbedCode) . ($autofit && $videoSource === 'vimeo' ? '<script src="https://player.vimeo.com/api/player.js"></script>' : '') . '</div></div>';
}
