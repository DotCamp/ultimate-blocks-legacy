...
class TableOfContents extends Component {
	...
	componentDidMount() {
		...
		const getHeadingBlocks = () => {
			...
			rootBlocks.forEach((block) => {
				if (block.name === 'core/heading') {
					...
					if (block.name === 'ub/advanced-heading') {
						...
						block.name === 'themeisle-blocks/advanced-heading'
					) {
						if (
							...
							newBlock.attributes = Object.assign(
								{},
								{
									content: Array.isArray(
										...
										: blockAttributes.content,
									level: Number(
										blockAttributes.element.charAt(1)
									),
					...
					if (block.innerBlocks.length > 0) {
						let internalHeadings = getDescendantBlocks(
							block
						).filter(
							(block) =>
								[
						...
						if (internalHeadings.length > 0) {
							internalHeadings = internalHeadings.map((h) => {
								switch (h.name) {
									case 'ub/advanced-heading':
										...
										if (
											typeof h.attributes.level !==
											'number'
										) {
											h.attributes.level = Number(
												h.attributes.level.charAt(1)
											);
										}
										break;
									...
									case 'generateblocks/headline':
										...
										h.attributes.level = [
											...
											? Number(
													h.attributes.element.charAt(
														1
													)
											  )
											: 0;
										if (
											Array.isArray(h.attributes.content)
										) {
											h.attributes.content =
												mergeRichTextArray(
													h.attributes.content
									...
									case 'ub/content-toggle-panel-block':
										h.attributes.content =
											h.attributes.panelTitle;
										h.attributes.level = Number(
											blockAttributes.titleTag.charAt(1)
										);
										h.attributes.anchor =
											h.attributes.toggleID;
							...
							internalHeadings.filter(
								(h) => h.attributes.level > 0
							);
						}
						...
						if (internalHeadings.length > 0) {
							...
							pageBreaks.push(
								...Array(internalHeadings.length).fill(pageNum)
							);
						}
					}
				}
			...
			if (
				JSON.stringify(this.state.breaks) !== JSON.stringify(pageBreaks)
			) {
		...
		const setHeadings = (checkIDs = true) => {
			...
			headers.forEach((heading, key) => {
				if (
					...
					if (removeDiacritics) {
						heading.anchor = filterDiacritics(
							heading.anchor
						).replace(
							/[\u0300-\u036F\u1AB0-\u1AFF\u1DC0-\u1DFF\u20D0-\u20FF]/g,
							''
					...
					if (
						...
						heading.anchor !==
							getBlock(heading.clientId).attributes.anchor
					) {
					...
					if (
						...
						heading.anchor !==
							getBlock(heading.clientId).attributes.toggleID
					) {
						updateBlockAttributes(heading.clientId, {
					...
					'disabled' in this.state.headers[i]
						? checkIDs
							? currentIDs.indexOf(header.clientId) > -1
								? this.state.headers[
										currentIDs.indexOf(header.clientId)
								  ].disabled
								: false
							...
							this.setState({
								headers: this.state.headers.map((hd, i) => {
									...
									return Object.assign({}, newHeaders[i], {
										disabled:
											newHeaders[i].disabled ||
											defaultReplacement.disabled,
										customContent:
		...
		if (this.state.hasIdMismatch) {
			...
			if (oldIDs.length === newIDs.length) {
				...
				for (let i = 0; i < replacementHeaders.length; i++) {
					if (
						headers[i].clientId !== replacementHeaders[i].clientId
					) {
						mismatchLocs.push(i);
					}
				}
				let replacements = JSON.parse(
					...
					newIDs.indexOf(a.clientId) > newIDs.indexOf(b.clientId)
						? 1
						: -1
				);
				...
				if (mismatchLocs.length < 1) {
					replacements = replacements.map((h, i) =>
						Object.assign({}, h, {
							disabled:
								headers[newIDs.indexOf(headers[i].clientId)]
									.disabled,
							customContent:
								headers[newIDs.indexOf(headers[i].clientId)]
									.customContent,
					...
					if (newIDs.length) {
						deletionSpots.forEach((index) => {
							if (index !== currentHeaders[index].index) {
								...
								Object.assign(
									currentHeaders[currentHeaders[index].index],
									{
										disabled:
											currentHeaders[index].disabled,
										customContent:
											currentHeaders[index].customContent,
									}
								);
							}
	...
	render() {
						...
						{isSelected && (
							<div className="ub_toc_button_container">
								{!item.disabled && (
									<button
										onClick={() => {
											...
											if (
												!revisedHeaders[item.index]
													.customContent
											) {
												revisedHeaders[
													item.index
												].customContent = revisedHeaders[
													item.index
												].content.replace(/<.+?>/g, '');
												this.setState({
								...
								>
									<FontAwesomeIcon
										icon={
											item.disabled ? faEye : faEyeSlash
										}
									/>
								</button>
							</div>
		...
		if (
			headers.length > 0 &&
			headers.filter((header) => allowedHeaders[header.level - 1])
				.length > 0
		) {
			return (
				<div
		...
		return (
			blockProp && (
				<p className="ub_table-of-contents-placeholder">
					{__(
						'Add a heading to begin generating the table of contents'
					)}
				</p>
			)
		);
					...
					<ToggleControl
						...
						onChange={() => {
							const tocInstances = getBlocks().filter(
								(block) =>
									block.name === 'ub/table-of-contents-block'
							);
...
export const editorDisplay = (props) => {
	const { setAttributes, canRemoveItemFocus, toggleCanRemoveItemFocus } =
		props;
	const {
		links,
		title,
