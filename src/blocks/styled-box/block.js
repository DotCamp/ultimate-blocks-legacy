...
const { RichText, BlockControls, MediaUpload, InspectorControls, PanelColorSettings } = wp.editor;
...
registerBlockType('ub/styled-box', {
	...
	edit: withState({editable: ''})(function(props) {
		const { attributes, setAttributes, isSelected, setState, editable } = props;
		...
		if (mode === 'notification') {
            ...
            blockToolbarExtras = columnCountToolbar;
            renderedBlock = renderedBlock = Array(text.length)
        .fill('')
        .map((_, i) => (
            <div
        ...
        inspectorExtras = <PanelColorSettings
                ...
                <Toolbar>
                    {['left', 'center', 'right', 'justify'].map(a => (
                        <IconButton
                            ...
                            onClick={_ => {
                                const columnNum = parseInt(editable.slice(editable.length-1));
                                if(editable.includes('title')){
                                    setAttributes({titleAlign: [...titleAlign.slice(0, columnNum), a, ...titleAlign.slice(columnNum+1)]})
                                }
                                else if(editable.includes('text')){
                                    setAttributes({textAlign: [...textAlign.slice(0, columnNum), a, ...textAlign.slice(columnNum+1)]})
                                }
